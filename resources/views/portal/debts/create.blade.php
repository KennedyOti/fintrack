...
@section('title', 'Add ' . ($isReceivable ? 'Receivable' : 'Payable') . ' - FinTrack')
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        ...
        <h4 class="mb-0">
            ...
            Add {{ $isReceivable ? 'Receivable' : 'Payable' }}
        </h4>
    </div>
...
<div class="row g-4">
    ...
    <div class="col-lg-8">
        <form method="POST"
              action="{{ $isReceivable ? route('debts.receivable.store') : route('debts.payable.store') }}"
                ...
                <div class="card-body p-4">
                    ...
                    <div class="row g-3 mb-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-select @error('invoice_id') is-invalid @enderror"
                                ...
                                {{-- Populated via JS based on client selection or can be static --}}
                            </select>
                            @error('invoice_id')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="input-group">
                                ...
                                <input type="number" step="0.01" min="0"
                                       ...
                                       placeholder="0.00"
                                       value="{{ old('original_amount') }}" required>
                    ...
                    <div class="mb-0">
                        ...
                        <textarea class="form-control @error('notes') is-invalid @enderror"
                                  ...
                                  placeholder="Any additional details about this {{ $type }}…">{{ old('notes') }}</textarea>
                        @error('notes')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
            ...
            <div class="d-flex gap-2">
                <button type="submit" class="btn btn-{{ $color }} px-4">
                    <i class="fas fa-save me-2"></i> Save {{ $isReceivable ? 'Receivable' : 'Payable' }}
                </button>
                <a href="{{ route('debts.'.$type.'.index') }}" class="btn btn-outline-secondary px-4">
                    Cancel
                </a>
            </div>
        </form>
    ...
    <div class="col-lg-4">
        @if($isReceivable)
        <div class="info-box border-success-subtle bg-success-subtle mb-3">
            <div class="d-flex gap-2">
                <i class="fas fa-lightbulb text-success mt-1"></i>
                <div>
                    <div class="fw-semibold text-success mb-1">About Receivables</div>
                    <p class="mb-0 small text-muted">
                        A <strong>receivable</strong> is money a client owes you for goods or services delivered.
                        Track it here to monitor your outstanding income.
                    </p>
                </div>
            </div>
        </div>
        <div class="info-box border-info-subtle bg-info-subtle mb-3">
            <div class="d-flex gap-2">
                <i class="fas fa-info-circle text-info mt-1"></i>
                <div>
                    <div class="fw-semibold text-info mb-1">Auto-created from Invoices</div>
                    <p class="mb-0 small text-muted">
                        When you create an invoice with status <em>Sent</em> or <em>Partial</em>,
                        a receivable is created automatically. Use this form for manual tracking.
                    </p>
                </div>
            </div>
        </div>
        @else
        <div class="info-box border-danger-subtle bg-danger-subtle mb-3">
            <div class="d-flex gap-2">
                <i class="fas fa-lightbulb text-danger mt-1"></i>
                <div>
                    <div class="fw-semibold text-danger mb-1">About Payables</div>
                    <p class="mb-0 small text-muted">
                        A <strong>payable</strong> is money you owe to a vendor, supplier, or creditor.
                        Tracking payables helps you manage cash flow and avoid late penalties.
                    </p>
                </div>
            </div>
        </div>
            ...
            <div class="card-body">
                ...
                <div class="d-flex flex-column gap-2">
                    <div class="d-flex align-items-center gap-2">
                        <span class="badge bg-warning-subtle text-warning border border-warning-subtle" style="width:80px;text-align:center">Pending</span>
                        <small class="text-muted">Not yet paid at all</small>
                    </div>
                    <div class="d-flex align-items-center gap-2">
                        <span class="badge bg-info-subtle text-info border border-info-subtle" style="width:80px;text-align:center">Partial</span>
                        <small class="text-muted">Partially paid</small>
                    </div>
                    <div class="d-flex align-items-center gap-2">
                        <span class="badge bg-success-subtle text-success border border-success-subtle" style="width:80px;text-align:center">Paid</span>
                        <small class="text-muted">Fully settled</small>
                    </div>
                    <div class="d-flex align-items-center gap-2">
                        <span class="badge bg-danger-subtle text-danger border border-danger-subtle" style="width:80px;text-align:center">Overdue</span>
                        <small class="text-muted">Past due date, unpaid</small>
                    </div>
                </div>
            </div>
        </div>
